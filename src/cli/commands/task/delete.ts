...
import { getTask, deleteTask } from "../../../api/tasks.ts";
...
export function registerDeleteCommand(task: Command): void {
  task
    .command("delete")
    ...
    .action(async (ids: string[], opts: { filter?: string; quiet?: boolean; dryRun?: boolean }) => {
      try {
        ...
        const results = await Promise.allSettled(resolved.map(async (id) => {
          ...
          try { await hooks?.emit("task.deleting", { task }); } catch { /* hook errors non-fatal */ }
          await deleteTask(id);
          try { await hooks?.emit("task.deleted", { task }); } catch { /* hook errors non-fatal */ }
        }));
        let success = 0;
        let failed = 0;
